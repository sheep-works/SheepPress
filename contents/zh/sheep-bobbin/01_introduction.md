...
其核心应用场景为翻译质检与校验，但根据 Prompt 提示词的不同，同样支持译文生成、单语校对、语气口吻重构等丰富任务。
通常情况下，它与在线 Web 应用 [SheepCombWeb](https://comb.lambuage.com/) 或 VS Code 扩展型翻译辅助工具 [SheepWeave](/zh/sheep-weave/) 配合使用。
...
1. 访问 <a href="https://storage.lambuage.com" target="_blank" rel="noopener noreferrer">下载页面</a> 获取 exe 安装程序。
2. 双击安装并启动程序。
...
### 使用 Google AI Studio / ChatGPT / DeepSeek
...
访问 [SheepCombWeb](https://comb.lambuage.com/)，从 **提取（Parse）** 页面开始处理。
...
## 从 SheepWeave 发送 AI 请求

在 SheepWeave 中进行翻译时，同样可以无缝调用 SheepBobbin 进行 AI 处理。
详情请参阅 [SheepWeave 中的 LLM / AI 深度联动](/zh/sheep-weave/08_LLM_usage)。
